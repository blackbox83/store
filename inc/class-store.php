...
if ( ! class_exists( 'Store' ) ) :
	...
	class Store {
		...
		public function setup() {
			...
			// Loads wp-content/languages/themes/store-it_IT.mo.
			load_theme_textdomain( 'store', trailingslashit( WP_LANG_DIR ) . 'themes/' );

			// Loads wp-content/themes/child-theme-name/languages/it_IT.mo.
			load_theme_textdomain( 'store', get_stylesheet_directory() . '/languages' );

			// Loads wp-content/themes/store/languages/it_IT.mo.
			...
			register_nav_menus( array(
				...
				'secondary'		=> __( 'Secondary Menu', 'store' ),
				'handheld'		=> __( 'Handheld Menu', 'store' ),
			) );
			...
			add_theme_support( 'custom-background', apply_filters( 'store_custom_background_args', array(
				'default-color' => apply_filters( 'store_default_background_color', 'ffffff' ),
				'default-image' => '',
			) ) );
		...
		public function widgets_init() {
			...
			$footer_widget_regions = apply_filters( 'store_footer_widget_regions', 4 );

			for ( $i = 1; $i <= intval( $footer_widget_regions ); $i++ ) {
				register_sidebar( array(
					'name' 				=> sprintf( __( 'Footer %d', 'store' ), $i ),
					'id' 				=> sprintf( 'footer-%d', $i ),
					'description' 		=> sprintf( __( 'Widgetized Footer Region %d.', 'store' ), $i ),
					'before_widget' 	=> '<aside id="%1$s" class="widget %2$s">',
					'after_widget' 		=> '</aside>',
					'before_title' 		=> '<h3>',
					'after_title' 		=> '</h3>',
		...
		public function scripts() {
			 ...
			 * Styles
			 */
			wp_enqueue_style( 'store-style', get_template_directory_uri() . '/style.css', '', $store_version );
			wp_style_add_data( 'store-style', 'rtl', 'replace' );

			/**
			 * Fonts
			 */
			$google_fonts = apply_filters( 'store_google_font_families', array(
			...
			wp_enqueue_style( 'store-fonts', $fonts_url, array(), null );

			/**
			 * Scripts
			 */
			wp_enqueue_script( 'store-navigation', get_template_directory_uri() . '/assets/js/navigation.min.js', array( 'jquery' ), '20120206', true );
			wp_enqueue_script( 'store-skip-link-focus-fix', get_template_directory_uri() . '/assets/js/skip-link-focus-fix.min.js', array(), '20130115', true );
		...
		public function child_scripts() {
			if ( is_child_theme() ) {
				wp_enqueue_style( 'store-child-style', get_stylesheet_uri(), '' );
			}
		}
		...
		public function body_classes( $classes ) {
			 ...
			 * Or take the red pill, filter store_make_me_cute and see how deep the rabbit hole goes...
			 */
			$cute = apply_filters( 'store_make_me_cute', false );
			...
			if ( ! is_active_sidebar( 'sidebar-1' ) ) {
				$classes[] = 'store-full-width-content';
			}
		...
		public function print_embed_styles() {
			...
			<style type="text/css">
				.wp-embed {
					...
					background-color: <?php echo store_adjust_color_brightness( $background_color, -7 ); ?> !important;
				}
